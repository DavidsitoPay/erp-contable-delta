...
-- DDL generado a partir de docs/data-dictionary.md (entregable 05/09)
-- Incluye las correcciones de la observación de auditoría (RN-12, SaldoCuentaPeriodo)
-- =====================================================================
...
-- NOTA (RN-08): esta tabla se llena exclusivamente desde la API (usuario_id viene del
-- token JWT). No se debe crear un trigger que intente poblar usuario_id automáticamente.
...
-- NOTA: sin columna "saldo". Se calcula siempre desde lineaasiento.
...
CREATE TABLE documentocxc (
    ...
    saldo_pendiente       DECIMAL(14,2) NOT NULL, -- RN-12: derivado, recalculado por trigger
    tipo_cambio_aplicado  DECIMAL(12,6),
...
CREATE TABLE pagoproveedorcabecera (
    ...
    metodo_pago          VARCHAR(30) NOT NULL,
    referencia_bancaria  VARCHAR(100),
    cuenta_bancaria_id   INT -- FK -> cuentabancaria(id)
);
...
CREATE TABLE cuentabancaria (
    ...
    -- NOTA (corrección de auditoría): sin columna "saldo" editable.
    -- El saldo en tiempo real se calcula desde movimientotesoreria;
    -- el saldo de cierre se consolida en saldocuentaperiodo.
);
...
-- Tabla nueva (corrección de auditoría): saldo consolidado e inmutable por periodo.
-- No se actualiza con UPDATE; un nuevo cierre inserta un nuevo registro.
...
-- =====================================================================
-- Fin de 01_tables.sql — ver 02_functions.sql, 03_triggers.sql,
-- 04_procedures.sql y 05_views.sql para la lógica que sostiene RN-01, RN-08 y RN-12.
-- =====================================================================
